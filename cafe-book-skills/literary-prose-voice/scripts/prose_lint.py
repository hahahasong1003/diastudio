...
RULES = [
    ...
    ("passive-double", ERROR,
     ...
     "능동으로 바꾼다. '보여진다' → '보인다'."),
    ("passive-causative-double", WARN,
     r"(높여|낮춰|넓혀|좁혀|세워|재워|채워|비워|키워|데워|맞춰|늦춰|갖춰|늘려|줄여|옮겨)"
     r"(져|졌|지)",
     "사동에 피동을 겹쳤다.",
     "'높여져 있다' → '높였다'. 펼쳐지다·켜지다처럼 본래 자동사인 말은 해당하지 않는다."),
    ("passive-by", WARN,
     r"에\s*의(해|하여)\s",
     "번역투 피동 구문이다.",
     "행위자를 주어로 세운다."),
    ...
    ("fiction-exposure", ERROR,
     r"(가상의|상상의\s*공간|실재하지\s*않|실제로\s*존재하지\s*않|허구(의|다|라)|검색해도)",
...
# 종결어미의 마지막 음절. 평서·의문·명령·청유·감탄을 모두 포함한다.
SENTENCE_ENDINGS = set("다라까요오자지죠네군걸야니세소마련가나냐람담랴게렴")
